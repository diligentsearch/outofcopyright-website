...
function saveFile(message){
	$.post( "//dev.outofcopyright.eu/node", { country: country, name: country+".json", action: 'update', file: JSON.stringify(file), message: message } );
}

function saveTraduction(langue, message){
	$.post( "//dev.outofcopyright.eu/node", { country: country, name: langue+".json", action: 'update', file: JSON.stringify(bibliotheque[langue]), message: message } );
}