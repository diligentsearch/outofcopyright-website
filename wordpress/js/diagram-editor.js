...
$(function(){
	...
	$('#drawSvg').attr('height', $( document ).height() - 32 - 48 - 56);
	...
	readFile(country, country+'.json', function(data){
		parseJSON(data);
		var result = idLink(2);
		readFile(country, languageChoosen+'.json', function(dataTrad){
...
function draw(){
	...
	$(".node").each(function () {
  	...
  	$('.node').click( function(){
    	...
    	$(this).find("rect").css("fill","#2196f3");
    	$(this).css("fill","#fff");
...
function formatD3Nodes(g){
	for(var i = 0; i < file.subgraph[typeOfWork].nodes.length; i++){
		...
		noeud.value.label = "";
		var strADecoupe = getTraduction(languageChoosen, node.text);
		var cutHere = 0;
...
function setQuestionInput(){
	$('#question').val(getTraduction(languageChoosen, nodeSelected.text));
}
...
function loadDatapoints(id){
	...
	$("#questionDatapoint").val(getTraduction(languageChoosen,'question_'+datapoint.id));
