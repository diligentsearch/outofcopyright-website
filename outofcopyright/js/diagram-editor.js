...
$(function(){
	...
	$.post( "/node", { country: country, name: country+".json", action: 'read' } )
		...
		$( "#addModifyDatapoint" ).click(function() {
			if($("#questionDatapoint").val() !== ""){
				...
				var hint = formatStringInput($("#hintDatapoint").val());
				var question = formatStringInput($("#questionDatapoint").val());
	...
	$('.popoverInfo').popover({placement:'bottom', html : true});
...
function resizeVerticaly(){
	...
	heightTypeOfWork = (height - 177) / 2;
	heightDatapoints = (height - 177) / 2;
